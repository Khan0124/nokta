...
const featureFlagSchemas = {
  ...
  update: Joi.object({
    ...
    rollout: Joi.object({
      ...
      roles: Joi.alternatives().conditional('strategy', {
        is: 'roles',
        then: Joi.array().items(Joi.string().max(64)).min(1).required(),
        otherwise: Joi.forbidden()
      }),
      branches: Joi.alternatives().conditional('strategy', {
        is: 'branches',
        then: Joi.array().items(commonSchemas.id).min(1).required(),
        otherwise: Joi.forbidden()
      }),
...
const adminDashboardSchemas = {
  ...
  reportQuery: Joi.object({
    ...
    startDate: Joi.date().iso().max('now').optional(),
    endDate: Joi.date().iso().max('now').optional(),
    type: Joi.string().valid('overview', 'sales', 'orders', 'drivers', 'pricing').default('overview')
  }).custom(validateDateRange),
  adoptionQuery: Joi.object({
    branchId: commonSchemas.id.optional(),
    startDate: Joi.date().iso().max('now').optional(),
    endDate: Joi.date().iso().max('now').optional()
  }).custom(validateDateRange)
