...
const DEFAULT_WIDGET_LAYOUT = [
  ...
  {
    id: 'dynamic-pricing',
    title: 'Dynamic Pricing Adoption',
    size: 'medium',
    roles: ['admin', 'manager'],
    metrics: ['pricing.adoptionRate', 'pricing.discountedOrders', 'pricing.influencedRevenue']
  }
];

const ROLE_PERMISSIONS = {
  admin: {
    widgets: ['sales-overview', 'active-orders', 'top-products', 'driver-performance', 'payments-mix', 'dynamic-pricing'],
    reports: ['overview', 'sales', 'orders', 'drivers', 'pricing']
  },
  manager: {
    widgets: ['sales-overview', 'active-orders', 'top-products', 'driver-performance', 'payments-mix', 'dynamic-pricing'],
    reports: ['overview', 'sales', 'orders', 'pricing']
  },
  cashier: {
...
router.get(
  '/pricing/adoption',
  validate(schemas.adminDashboard.adoptionQuery, 'query'),
  asyncHandler(async (req, res) => {
    const filters = resolveFilters(req);
    const data = await loadDynamicPricingAdoption(filters);
